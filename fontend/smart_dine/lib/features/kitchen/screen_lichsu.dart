...
class ScreenHistory extends ConsumerStatefulWidget {
  final int? companyId;
  final int? branch;
  const ScreenHistory({super.key, this.companyId, this.branch});
...
class _ScreenHistoryState extends ConsumerState<ScreenHistory> {
  ...
  void initState() {
    super.initState();
    Future.microtask(_loadAllOrderItems);
    Future.microtask(_loadItemsByCompanyId);
    Future.microtask(_loadOrders);
    Future.microtask(_loadTablesByBranchId);
  }

  Future<void> _loadAllOrderItems() async {
    await ref
        .read(kitchenOrderNotifierProvider.notifier)
        .loadOrderItemsByBranch(branchId: widget.branch ?? 1);
  }
  ...
  Widget _buildOrderList(List<OrderItem> orders) {
    ...
    return ListView.separated(
      ...
      itemBuilder:
          (context, index) => _buildOrderCard(context, sortedOrders[index]),
    );
  }

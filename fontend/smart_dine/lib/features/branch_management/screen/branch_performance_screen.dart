...
class _BranchPerformanceScreenState extends ConsumerState<BranchPerformanceScreen> {
    ...
    return Scaffold(
      ...
      body: performanceAsync.when(
        ...
        error: (error, stackTrace) =>
            _buildErrorState(error, isDark, cardColor, textColor, currentBranchId),
        data: (data) {
          ...
          return _buildContent(data, isDark, cardColor, textColor, currentBranchId);
        },
      ),
    );
  }
  ...
  Widget _buildContent( data, bool isDark, Color cardColor,
      Color textColor, int branchId) {
    final performances = data.employeeList ?? [];
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Tổng quan
            //------------------------------------------------------------------
            Text('Tổng quan chi nhánh', style: Style.fontTitleMini.copyWith(color: textColor)),
            const SizedBox(height: 12),

            Row(
              children: [
                Expanded(
                  child: _buildStatCard('Tổng đơn hàng', '1,845', Icons.shopping_cart, Colors.blue,
                      isDark, cardColor, textColor),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: _buildStatCard('Doanh thu', '165 triệu', Icons.attach_money, Colors.green,
                      isDark, cardColor, textColor),
                ),
              ],
            ),
            ...
            Row(
              children: [
                Expanded(
                  child: _buildStatCard('Khách hàng', '1,520', Icons.people, Colors.orange, isDark,
                      cardColor, textColor),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: _buildStatCard('Đánh giá TB', '4.8★', Icons.star, Colors.amber, isDark,
                      cardColor, textColor),
                ),
              ],
            ),
            ...
            // Top nhân viên
            //------------------------------------------------------------------
            Text('Top nhân viên', style: Style.fontTitleMini.copyWith(color: textColor)),
            const SizedBox(height: 12),
              ...
              child: Column(
                children: [
                  ...
                  const Divider(),
                    ...
                    ...performances.map((e) => Row(
                      children: [
                        Expanded(
                          child: Text(e['name'] ?? '-',
                            style: Style.fontContent.copyWith(color: textColor))),
                        Expanded(
                          child: Text(e['totalServed']?.toString() ?? '-',
                            style: Style.fontContent.copyWith(color: textColor))),
                        Expanded(
                          child: Text(e['tips']?.toString() ?? '-',
                            style: Style.fontContent.copyWith(color: textColor))),
                      ],
                      )),
                ],
            ...
            //------------------------------------------------------------------
            Text('Doanh thu theo giờ', style: Style.fontTitleMini.copyWith(color: textColor)),
            const SizedBox(height: 12),
            ...
            // Hiệu xuất nhân viên
            //------------------------------------------------------------------
            Text('Hiệu xuất nhân viên', style: Style.fontTitleMini.copyWith(color: textColor)),
            const SizedBox(height: 12),
            ...
            Container(
              ...
              child: Column(
                children: [
                  ...performances.map((employee) => Column(
                        children: [
                          _buildEmployeeRow(
                            employee['name'] ?? '-',
                          ...
                          if (performances.indexOf(employee) < performances.length - 1)
            ...
            // Trips thức chart
            //------------------------------------------------------------------
            Text('Trips thức', style: Style.fontTitleMini.copyWith(color: textColor)),
            const SizedBox(height: 12),
        ...
        children: [
          ...
          Text(title,
              style: Style.fontCaption
                  .copyWith(color: isDark ? Colors.grey[400] : Style.textColorGray)),
          const SizedBox(height: 4),
          Text(value, style: Style.fontTitleMini.copyWith(color: textColor, fontSize: 20)),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          Text('Chưa có dữ liệu hiệu xuất',
              style: Style.fontTitle.copyWith(color: textColor, fontWeight: FontWeight.bold)),
          const SizedBox(height: 12),
          Text(
  ...
  Widget _buildEmployeeRow(
      String name, String v1, String v2, String v3, Color textColor) {
    return Row(
      ...
      children: [
        ...
        Expanded(child: Text(v1, textAlign: TextAlign.center, style: TextStyle(color: textColor))),
        Expanded(child: Text(v2, textAlign: TextAlign.center, style: TextStyle(color: textColor))),
        Expanded(child: Text(v3, textAlign: TextAlign.center, style: TextStyle(color: textColor))),
      ],
    );
  }
  ...
  Widget _buildErrorState(
      Object error, bool isDark, Color cardColor, Color textColor, int branchId) {
    return Container(
      padding: const EdgeInsets.all(32),
      margin: const EdgeInsets.all(16),
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Text('Lỗi tải dữ liệu hiệu suất',

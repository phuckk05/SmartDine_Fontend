...
import 'package:mart_dine/providers_owner/system_stats_provider.dart' show ownerProfileProvider;
import 'package:mart_dine/providers_owner/category_provider.dart';
import 'package:mart_dine/providers_owner/item_provider.dart'; // SỬA: Import item_provider để dùng logic xóa mới
...
class ScreenCategoryManagement extends ConsumerStatefulWidget {
  const ScreenCategoryManagement({super.key});
  @override
  ConsumerState<ScreenCategoryManagement> createState() =>
      _ScreenCategoryManagementState();
}

class _ScreenCategoryManagementState extends ConsumerState<ScreenCategoryManagement> {
    ...
    showDialog<bool>( // Nhận giá trị trả về (true nếu nhấn xóa)
      ...
      if (wantsToDelete == true) {
        // Bây giờ hiển thị dialog xác nhận với context gốc của màn hình
              ...
              if (confirm == true) {
                try {
                  // SỬA: Gọi notifier mới để xử lý xóa category và các liên kết
                  await ref
    ...
    showDialog(
      ...
      builder:
          (context) => AddCategoryModal(
            // TODO: Sửa provider cho việc thêm mới
  ...
  Widget _menuCategoryItem(Category category) {
    return GestureDetector(
      // BỎ: Hành động onTap không còn cần thiết vì màn hình này chỉ để quản lý danh sách.
      // Việc quản lý món ăn trong nhóm được thực hiện thông qua quản lý Menu.
      onTap: () {},
